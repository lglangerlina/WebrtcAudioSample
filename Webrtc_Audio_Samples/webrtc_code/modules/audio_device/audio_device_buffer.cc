...
//     wwc main_thread_checker_
  RTC_DCHECK_RUN_ON(&main_thread_checker_);
//    webrtc::webrtc_seq_check_impl::SequenceCheckerScope seq_check_scope(&main_thread_checker_);
...
void AudioDeviceBuffer::SetVQEData(int play_delay_ms, int rec_delay_ms) {
  ...
  RTC_LOG(INFO) << "SetVQEData-------------0-play_delay_ms_: "<<play_delay_ms_<<"  rec_delay_ms_:"<<rec_delay_ms_;
}
