...
AudioDeviceWindowsCore::AudioDeviceWindowsCore()
  ...
  _notificationClient = new CIMMNotificationClient(nullptr);
  _ptrEnumerator->RegisterEndpointNotificationCallback(_notificationClient);
...
AudioDeviceWindowsCore::~AudioDeviceWindowsCore() {
  ...
  if (_ptrEnumerator) {
    _ptrEnumerator->UnregisterEndpointNotificationCallback(_notificationClient);
  }
...
void AudioDeviceWindowsCore::RegisterDeviceCallback(
    AudioDeviceState* deviceCallBack) {

  // CoreAudio device states
  if (NULL != _notificationClient) {
    _notificationClient->RegisterCallback(deviceCallBack);
  }
}

webrtc::CIMMNotificationClient::CIMMNotificationClient(AudioDeviceState* deviceCallback)
    : _deviceCallback(deviceCallback), _cRef(1) {}

webrtc::CIMMNotificationClient::~CIMMNotificationClient() {

  _deviceCallback = NULL;
}

ULONG __stdcall webrtc::CIMMNotificationClient::AddRef() {
  return InterlockedIncrement(&_cRef);
}

ULONG __stdcall webrtc::CIMMNotificationClient::Release() {
...
void webrtc::CIMMNotificationClient::RegisterCallback(
    AudioDeviceState* deviceCallback) {

  _deviceCallback = deviceCallback;
}

HRESULT __stdcall webrtc::CIMMNotificationClient::OnDefaultDeviceChanged(
    EDataFlow flow,
    ERole role,
...
HRESULT __stdcall webrtc::CIMMNotificationClient::OnDeviceAdded(LPCWSTR pwstrDeviceId) {

  RTC_LOG(INFO) << __FUNCTION__ << "deviceid=" << pwstrDeviceId;
  return S_OK;
}

HRESULT __stdcall webrtc::CIMMNotificationClient::OnDeviceRemoved(
...
HRESULT __stdcall webrtc::CIMMNotificationClient::OnDeviceStateChanged(
    LPCWSTR pwstrDeviceId,
    DWORD dwNewState) {
  int convResult = 0;
  char deviceId[256] = {0};
  AudioDeviceState::NewDeviceState newState;

  convResult = WideCharToMultiByte(CP_UTF8, 0, pwstrDeviceId, -1, deviceId,
                                   sizeof(deviceId), NULL, NULL);

  if (dwNewState == DEVICE_STATE_ACTIVE) {
    newState = AudioDeviceState::NewDeviceState::Enabled;
  } else {
    newState = AudioDeviceState::NewDeviceState::Disabled;
  }
  if (convResult > 0) {
    RTC_LOG(INFO) << __FUNCTION__ << "deviceid=" << deviceId
                  << ", newstate=" << newState;
    if (NULL != _deviceCallback) {
      _deviceCallback->OnDeviceStateChanged(deviceId, newState);
	}
  }
...
HRESULT __stdcall webrtc::CIMMNotificationClient::OnPropertyValueChanged(
    LPCWSTR pwstrDeviceId,
...
HRESULT __stdcall webrtc::CIMMNotificationClient::QueryInterface(
    const IID& riid,
    void** ppvObject) {
